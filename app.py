...
from flask import Flask, request, Response # <--- MODIFIED HERE
...
# (The entire middle of the file is unchanged)
responses = {
    'en': {
...
def chat():
    ...
    if current_state:
        if current_state == 'awaiting_lang_choice':
            if '1' in user_message:
                ...
                # The old return statement: return str(response)
                # We need to return the full Response object here too
    ...
    # --- MODIFIED HERE ---
    # We now return a full Response object with the correct 'mimetype'
    return Response(str(response), mimetype='text/xml')
